...
void TransInformation::dijkstra_Cost(const std::string &startCity, const std::string &endCity)
{
    ...
    std::vector<int> bestPidForCity; // bestPidForCity[v] = 使 distance[v] 最优的入队记录 pid
    int pushCount = 0;
    ...
    distance[start] = 0;
    arriveTime[start] = 0;
    bestPidForCity.resize(cityCount + 5, -1);
    ...
    while (!Q.empty())
    {
        ...
        for (auto &i : cityList[u.id - 1].trans)
        {
            ...
            if (newCost < distance[v] || newArrive < arriveTime[v])
            {
                ...
                {
                    distance[v] = newCost;
                    bestPidForCity[v] = myPid; // 记录使费用最优的路径
                }
    ...
    // 从终点回溯构建路径：bestPidForCity[endId] 给出了达到最小费用的那条路径
