...
private:
    ...
    Time startdep;
    void init();
    void dijkstra_Cost(const std::string &startCity);
    void print_Cheapest_Path(const std::string &startCity, const std::string &nowCity);

    void dijkstra_Time(const std::string &startCity);
    void print_Fastest_Path(const std::string &startCity, const std::string &nowCity);

    void BFS_Least_Transfer(const std::string &startCity);
    void print_Least_Transfer_Path(const std::string &startCity, const std::string &nowCity);
...
public:
    ...
    void searchCheapest(const std::string &startCity, const std::string &endCity)
    {
        ...
        print_Cheapest_Path(startCity, endCity);
    ...
    void searchLeastTransfer(const std::string &startCity, const std::string &endCity)
    {
        ...
        print_Least_Transfer_Path(startCity, endCity);
...
void TransInformation::dijkstra_Cost(const std::string &startCity)
{
    init();
    ...
    while (!Q.empty())
    {
        ...
        for (auto &i : cityList[u.id - 1].trans)
        {
            ...
            int departure = i.startTime.toMinutes();
            if (departure < arriveTime[u.id])
                continue;

            if (distance[v] > i.cost + u.w)
            {
                distance[v] = i.cost + u.w;
                arriveTime[v] = i.endTime.toMinutes();
...
void TransInformation::print_Cheapest_Path(const std::string &startCity, const std::string &nowCity)
{
    if (pre[cityID[nowCity]].startCity != startCity)
        print_Cheapest_Path(startCity, pre[cityID[nowCity]].startCity);
...
void TransInformation::BFS_Least_Transfer(const std::string &startCity)
{
    init();
    ...
    while (!Q.empty())
    {
        ...
        for (auto &i : cityList[u - 1].trans)
        {
            ...
            else if (distance[v] == distance[u] + 1 && arriveTime[v] > arr)
            {
                arriveTime[v] = arr;
                pre[v] = i;
            }
...
void TransInformation::print_Least_Transfer_Path(const std::string &startCity, const std::string &nowCity)
{
    if (pre[cityID[nowCity]].startCity != startCity)
        print_Least_Transfer_Path(startCity, pre[cityID[nowCity]].startCity);
