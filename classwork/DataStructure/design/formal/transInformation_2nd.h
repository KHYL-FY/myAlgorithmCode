...
void TransInformation::dijkstra_Cost(const std::string &startCity, const std::string &endCity)
{
    ...
    struct costNode
    {
        int id, cost, arrive, pid;
        costNode(int a, int b, int c, int p = -1) : id(a), cost(b), arrive(c), pid(p) {}
        bool operator<(const costNode &a) const { return cost > a.cost; }
    };
    ...
    while (!Q.empty())
    {
        ...
        if (u.cost > distance[u.id] && u.arrive > arriveTime[u.id])
            continue;
        ...
        for (auto &i : cityList[u.id - 1].trans)
        {
            ...
            if (departure < u.arrive)
                continue;

            int newCost = u.cost + i.cost;
            int newArrive = i.endTime.toMinutes();

            if (newCost < distance[v] || newArrive < arriveTime[v])
            {
                int myPid = pushCount++;
