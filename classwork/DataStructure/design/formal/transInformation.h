...
void TransInformation::dijkstra_Cost(const std::string &startCity, const std::string &endCity)
{
    ...
    /*1. 初始化：labels[start] = [(0, 0, -1)]，优先队列 Q = [(start, 0, 0)]

    2. while Q 非空：
       a. 弹出费用最小的标签 u = (city, cost, arrive)
       b. 若 u 被 labels[city] 中其他标签支配 → 跳过
       c. 对 city 的每条出边 e = (city → v, dep, arr, edgeCost)：
          - 若 arrive > dep → 赶不上，跳过
          - 新标签：newCost = cost + edgeCost, newArrive = arr
          - 若新标签被 labels[v] 中某标签支配 → 跳过
          - 否则：
            ① 删除 labels[v] 中被新标签支配的旧标签
            ② 将新标签加入 labels[v]
            ③ Q.push(v, newCost, newArrive)

    3. 终点标签中 cost 最小的那条即为最优路径*/
    while (!Q.empty())
    {
        costNode u = Q.top();
        Q.pop();
